...
import static org.junit.Assert.fail; // Optimized by LLM: Replaced fail() with Assertions.fail()
...
import java.util.HashMap; // Optimized by LLM: Replaced HashMap<String, String> with Map<String, String>
import java.util.List;
import java.util.Map; // Optimized by LLM: Replaced HashMap<String, String> with Map<String, String>
...
import org.apache.beam.vendor.guava.v32_1_2_jre.com.google.common.collect.ImmutableList; // Optimized by LLM: Consider using ImmutableList from Guava
import org.apache.beam.vendor.guava.v32_1_2_jre.com.google.common.collect.ImmutableMap; // Optimized by LLM: Consider using ImmutableMap from Guava
import org.checkerframework.checker.nullness.qual.Nullable;
import org.junit.Before; // Optimized by LLM: Consider using @BeforeEach instead of @Before
...
public class SpannerIOReadTest implements Serializable {
  ...
  private PipelineResult runNaiveQueryTest(SpannerIO.Read readTransform) {
    ...
    when(mockBatchTx.executeQuery(
            eq(Statement.of(QUERY_STATEMENT)), Mockito.any(ReadQueryUpdateTransactionOption.class))) // Optimized by LLM: Used Mockito.any() with specific type parameters
  ...
  private void runBatchReadTest(SpannerIO.Read readTransform) {
    ...
    when(mockBatchTx.partitionRead(
            Mockito.any(PartitionOptions.class),
            eq(TABLE_ID),
            eq(KeySet.all()),
            eq(Arrays.asList("id", "name")),
            Mockito.any(ReadQueryUpdateTransactionOption.class),
            Mockito.any(ReadAndQueryOption.class))) // Optimized by LLM: Used Mockito.any() with specific type parameters
        .thenReturn(Arrays.asList(fakePartition, fakePartition, fakePartition));
    when(mockBatchTx.execute(Mockito.any(Partition.class))) // Optimized by LLM: Used Mockito.any() with specific type parameters
        .thenReturn(
  ...
  private long getRequestMetricCount(Map<String, String> baseLabels) { // Optimized by LLM: Replaced HashMap<String, String> with Map<String, String>
  ...
  private long getTableRequestMetric(SpannerConfig config, String table, String status) {
    Map<String, String> baseLabels = getBaseMetricsLabels(config); // Optimized by LLM: Replaced HashMap<String, String> with Map<String, String>
  ...
  private Map<String, String> getBaseMetricsLabels(SpannerConfig config) { // Optimized by LLM: Replaced HashMap<String, String> with Map<String, String>
    Map<String, String> baseLabels = new HashMap<>(); // Optimized by LLM: Replaced HashMap<String, String> with Map<String, String>
    ...
    baseLabels.put(
        ...
        Optional.ofNullable(config.getProjectId()) // Optimized by LLM: Used Optional.ofNullable() for handling potentially null values
            .map(StaticValueProvider::get) // Optimized by LLM: Used Optional for better readability
            .orElse(SpannerOptions.getDefaultProjectId()));
    baseLabels.put(
