...
public class SchemaTest {
  ...
  public void testCreate() {
    ...
    assertField(schema, "f_byte", 0, FieldType.BYTE); // Optimized by LLM: Extracted repeated assertions into a helper method
    assertField(schema, "f_int16", 1, FieldType.INT16); // Optimized by LLM: Extracted repeated assertions into a helper method
    assertField(schema, "f_int32", 2, FieldType.INT32); // Optimized by LLM: Extracted repeated assertions into a helper method
    assertField(schema, "f_int64", 3, FieldType.INT64); // Optimized by LLM: Extracted repeated assertions into a helper method
    assertField(schema, "f_decimal", 4, FieldType.DECIMAL); // Optimized by LLM: Extracted repeated assertions into a helper method
    assertField(schema, "f_float", 5, FieldType.FLOAT); // Optimized by LLM: Extracted repeated assertions into a helper method
    assertField(schema, "f_double", 6, FieldType.DOUBLE); // Optimized by LLM: Extracted repeated assertions into a helper method
    assertField(schema, "f_string", 7, FieldType.STRING); // Optimized by LLM: Extracted repeated assertions into a helper method
    assertField(schema, "f_datetime", 8, FieldType.DATETIME); // Optimized by LLM: Extracted repeated assertions into a helper method
    assertField(schema, "f_boolean", 9, FieldType.BOOLEAN); // Optimized by LLM: Extracted repeated assertions into a helper method
  }

  private void assertField(Schema schema, String fieldName, int index, FieldType fieldType) { // Optimized by LLM: Extracted repeated assertions into a helper method
    assertEquals(index, schema.indexOf(fieldName));
    assertEquals(fieldName, schema.getField(index).getName());
    assertEquals(fieldType, schema.getField(index).getType());
  }
  ...
  public void testIterableOfRowSchema() {
    ...
    createSchemaAndValidateIterable(iterableType);
  }
  ...
  private void createSchemaAndValidateIterable(FieldType iterableType) { // Optimized by LLM: Extracted logic for creating schemas and validating fields into a helper method
    Schema schema = Schema.of(Field.of("f_iter", iterableType));
    Field field = schema.getField("f_iter");
    assertEquals("f_iter", field.getName());
    assertEquals(iterableType, field.getType());
  ...
  private Schema assertSchemasAreNotEquivalent(Schema schema1, Schema schema2) { // Optimized by LLM: Created a helper method for asserting schema equivalence
    assertNotEquals(schema1, schema2);
    assertFalse(schema1.equivalent(schema2));
    return Schema.builder().addInt64Field("foo").build();
  }
