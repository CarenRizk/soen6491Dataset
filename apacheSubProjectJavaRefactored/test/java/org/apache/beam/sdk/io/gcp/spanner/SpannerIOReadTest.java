...
public class SpannerIOReadTest implements Serializable {
  ...
  public void setUp() throws Exception {
    ...
    mockBatchTx = Mockito.mock(BatchReadOnlyTransaction.class, Mockito.withSettings().serializable());
  ...
  private void runBatchReadTest(SpannerIO.Read readTransform) {
    ...
    when(mockBatchTx.partitionRead(
        ...
        .thenReturn(List.of(fakePartition, fakePartition, fakePartition));
  ...
  private void checkMessage(String substring, String message) {
    if (message != null) {
  ...
  private long getTableRequestMetric(SpannerConfig config, String table, String status) {
    Map<String, String> baseLabels = getBaseMetricsLabels(config);
  ...
  private Map<String, String> getBaseMetricsLabels(SpannerConfig config) {
