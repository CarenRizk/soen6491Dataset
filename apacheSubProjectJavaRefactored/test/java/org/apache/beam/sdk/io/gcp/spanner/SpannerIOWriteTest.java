...
// Optimized by LLM: Consolidated prepareColumnMetadata and preparePgColumnMetadata
  static void prepareColumnMetadata(ReadOnlyTransaction tx, List<Struct> rows, boolean isPg) {
    createTableSchemaType(tx, rows, (Statement st) -> 
        st.getSql().contains("information_schema.columns") && (!isPg || st.getSql().contains("'public'")));
  }
...
// Optimized by LLM: Moved createTableSchemaType to a more appropriate location
