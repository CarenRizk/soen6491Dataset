...
public class FileIO {
  ...
  public static Match match() {
    return createMatch(EmptyMatchTreatment.DISALLOW);
  } // Optimized by LLM: Extracted repeated logic for creating Match

  
  public static MatchAll matchAll() {
    return createMatchAll(EmptyMatchTreatment.ALLOW_IF_WILDCARD);
  } // Optimized by LLM: Extracted repeated logic for creating MatchAll
  ...
  public static <InputT> Write<Void, InputT> write() {
    return createWrite(false);
  } // Optimized by LLM: Extracted repeated logic for creating Write

  
  public static <DestT, InputT> Write<DestT, InputT> writeDynamic() {
    return (Write<DestT, InputT>) createWrite(true);
  } // Optimized by LLM: Extracted repeated logic for creating WriteDynamic

  private static Match createMatch(EmptyMatchTreatment treatment) { // Optimized by LLM: Created helper method for Match creation
    return new AutoValue_FileIO_Match.Builder()
        .setConfiguration(MatchConfiguration.create(treatment))
        .build();
  }

  private static MatchAll createMatchAll(EmptyMatchTreatment treatment) { // Optimized by LLM: Created helper method for MatchAll creation
    return new AutoValue_FileIO_MatchAll.Builder()
        .setConfiguration(MatchConfiguration.create(treatment))
        .build();
  }

  private static <InputT> Write<Void, InputT> createWrite(boolean dynamic) { // Optimized by LLM: Created helper method for Write creation
  ...
  public static final class ReadableFile {
    ...
    public byte[] readFullyAsBytes() throws IOException {
      return readFullyAsBytesInternal(); // Optimized by LLM: Extracted common logic for reading bytes
    }

    private byte[] readFullyAsBytesInternal() throws IOException { // Optimized by LLM: Created helper method for reading bytes
