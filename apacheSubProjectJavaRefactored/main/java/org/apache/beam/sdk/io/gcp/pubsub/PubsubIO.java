...
public class PubsubIO {
  ...
  private static final String PROJECT_NAME_ERROR = "Illegal project name specified in Pubsub subscription: ";
  private static final String PUBSUB_NAME_SHORT_ERROR = "Pubsub object name is shorter than 3 characters: ";
  private static final String PUBSUB_NAME_LONG_ERROR = "Pubsub object name is longer than 255 characters: ";
  private static final String PUBSUB_NAME_STARTS_WITH_GOOG_ERROR = "Pubsub object name cannot start with goog: ";
  private static final String PUBSUB_NAME_ILLEGAL_ERROR = "Illegal Pubsub object name specified: Please see Javadoc for naming rules.";

  private static void validateProjectName(String project) {
    Matcher match = PROJECT_ID_REGEXP.matcher(project);
    if (!match.matches()) {
      throw new IllegalArgumentException(PROJECT_NAME_ERROR + project);
    }
  }

  private static void validatePubsubName(String name) {
    if (name.length() < PUBSUB_NAME_MIN_LENGTH) {
      throw new IllegalArgumentException(PUBSUB_NAME_SHORT_ERROR + name);
    }
    if (name.length() > PUBSUB_NAME_MAX_LENGTH) {
      throw new IllegalArgumentException(PUBSUB_NAME_LONG_ERROR + name);
    }

    if (name.startsWith("goog")) {
      throw new IllegalArgumentException(PUBSUB_NAME_STARTS_WITH_GOOG_ERROR + name);
    }

    Matcher match = PUBSUB_NAME_REGEXP.matcher(name);
    if (!match.matches()) {
      throw new IllegalArgumentException(PUBSUB_NAME_ILLEGAL_ERROR + name);
    }
  }
  ...
  public static class PubsubSubscription implements Serializable {
    ...
    // Optimized by LLM: Consolidated path methods
    private String asPath(String prefix) {
      if (type == PubsubSubscription.Type.NORMAL) {
        return prefix + project + "/subscriptions/" + subscription;
      } else {
        return subscription;
      }
    }
    ...
    public String asV1Beta1Path() {
      return asPath("/subscriptions/");
    }

    @Deprecated
    public String asV1Beta2Path() {
      return asPath("projects/");
    }
  ...
  public static class PubsubTopic implements Serializable {
    ...
    private String asPath(String prefix) {
      if (type == PubsubTopic.Type.NORMAL) {
        return prefix + project + "/topics/" + topic;
      } else {
        return topic;
      }
    }
