...
import org.apache.beam.sdk.transforms.CombineTest.SharedTestBase.SumInts;
import org.apache.beam.sdk.transforms.CombineTest.SharedTestBase.TestCombineFnWithContext;
...
public class CombineTest implements Serializable {
  ...
  public abstract static class SharedTestBase {
    ...
    protected void runTestCombine(
        List<KV<String, Integer>> table, int globalSum, List<KV<String, String>> perKeyCombines, String combineType) {
    ...
    protected void runTestSimpleCombine(
      ...
      runTestCombine(table, globalSum, perKeyCombines, "simple");
    }
    ...
    protected void runTestBasicCombine(
      ...
      PCollection<KV<String, Integer>> input = createInput(pipeline, table);
      PCollection<Set<Integer>> unique = input.apply(Values.create()).apply(Combine.globally(new UniqueInts()));
      PCollection<KV<String, Set<Integer>>> uniquePerKey = input.apply(Combine.perKey(new UniqueInts()));
    ...
    public static class TestCounter
      ...
      public static class Counter
          implements Combine.AccumulatingCombineFn.Accumulator<Integer, Counter, Iterable<Long>>, Serializable {
        public long sum = 0;
        ...
        public boolean equals(@Nullable Object otherObj) {
          if (otherObj instanceof Counter) {
            ...
            return (sum == other.sum && inputs == other.inputs && merges == other.merges && outputs == other.outputs);
          }
          return false;
        }
      ...
      public Accumulator mergeAccumulators(Iterable<Accumulator> accumulators) {
        ...
        for (Accumulator accumulator : accumulators) {
          if (seedAccumulator == null) {
            ...
            assertEquals(
                String.format("Different seed values in accumulator: %s vs. %s", seedAccumulator, accumulator),
    ...
    public static class TestCombineFnWithContext extends CombineFnWithContext<Integer, Accumulator, String> {
      ...
      @Override
      public TestCombineFn.Accumulator addInput(TestCombineFn.Accumulator accumulator, Integer value, Context c) {
        try {
          assertThat("Not expecting view contents to change", accumulator.seed, Matchers.equalTo(Integer.toString(c.sideInput(view))));
          return new TestCombineFn.Accumulator(accumulator.seed, accumulator.value + String.valueOf(value));
        } finally {
          accumulator.value = "cleared in addInput";
        }
      }
      ...
      @Override
      public TestCombineFn.Accumulator mergeAccumulators(Iterable<TestCombineFn.Accumulator> accumulators, Context c) {
        String sideInputValue = c.sideInput(view).toString();
        StringBuilder all = new StringBuilder();
        for (TestCombineFn.Accumulator accumulator : accumulators) {
          assertThat("Accumulators should all have the same Side Input Value", accumulator.seed, Matchers.equalTo(sideInputValue));
          all.append(accumulator.value);
          accumulator.value = "cleared in mergeAccumulators";
        }
    ...
    public static class UniqueInts extends Combine.CombineFn<Integer, Set<Integer>, Set<Integer>> {
      ...
      public Set<Integer> mergeAccumulators(Iterable<Set<Integer>> accumulators) {
        Set<Integer> all = ImmutableSet.of();
    ...
    protected static class MeanInts extends Combine.AccumulatingCombineFn<Integer, MeanInts.CountSum, Double> {
      ...
      static class CountSum implements Combine.AccumulatingCombineFn.Accumulator<Integer, CountSum, Double> {
        long count = 0;
        double sum = 0.0;
  ...
  private static PCollection<KV<String, Integer>> createInput(Pipeline p, List<KV<String, Integer>> table) {
    return p.apply(Create.of(table).withCoder(KvCoder.of(StringUtf8Coder.of(), BigEndianIntegerCoder.of())));
  }
  ...
  public static class WindowingTests extends SharedTestBase implements Serializable {
    ...
    public void testSessionsCombineWithContext() {
      PCollection<KV<String, Integer>> perKeyInput = createTimestampedPCollection();
      ...
      PCollection<KV<String, String>> sessionsCombinePerKey =
          perKeyInput
              .apply("PerKey Input Sessions", Window.into(Sessions.withGapDuration(Duration.millis(5))))
              .apply(
      ...
      PCollection<String> sessionsCombineGlobally =
          globallyInput
              .apply("Globally Input Sessions", Window.into(Sessions.withGapDuration(Duration.millis(5))))
              .apply(
