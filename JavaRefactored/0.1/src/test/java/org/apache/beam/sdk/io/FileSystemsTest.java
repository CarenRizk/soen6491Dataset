...
public class FileSystemsTest {
  ...
  public void testGetLocalFileSystem() throws Exception {
    ...
    assertTrue(getLocalFileSystem("~home/") instanceof LocalFileSystem); // Optimized by LLM: Replace repeated calls with a helper method
    assertTrue(getLocalFileSystem("file://home") instanceof LocalFileSystem); // Optimized by LLM: Replace repeated calls with a helper method
    assertTrue(getLocalFileSystem("FILE://home") instanceof LocalFileSystem); // Optimized by LLM: Replace repeated calls with a helper method
    assertTrue(getLocalFileSystem("File://home") instanceof LocalFileSystem); // Optimized by LLM: Replace repeated calls with a helper method
    if (SystemUtils.IS_OS_WINDOWS) {
      assertTrue(getLocalFileSystem("c:\\home\\") instanceof LocalFileSystem); // Optimized by LLM: Replace repeated calls with a helper method
    }
  }

  private LocalFileSystem getLocalFileSystem(String path) throws Exception { // Optimized by LLM: Create a helper method to reduce redundancy
    return (LocalFileSystem) FileSystems.getFileSystemInternal(toLocalResourceId(path).getScheme());
  }
  ...
  public void testDeleteIgnoreMissingFiles() throws Exception {
    ...
    FileSystems.delete(createResourceIdList(ImmutableList.of(existingPath, nonExistentPath), false)); // Optimized by LLM: Extract creation of ResourceId lists
  }
  ...
  public void testCopyThrowsNoSuchFileException() throws Exception {
    ...
    FileSystems.copy(
        createResourceIdList(ImmutableList.of(existingPath, nonExistentPath), false), // Optimized by LLM: Extract creation of ResourceId lists
        createResourceIdList(ImmutableList.of(destPath1, destPath2), false)); // Optimized by LLM: Extract creation of ResourceId lists
  }
  ...
  public void testCopyIgnoreMissingFiles() throws Exception {
    ...
    FileSystems.copy(
        createResourceIdList(ImmutableList.of(srcPath1, nonExistentPath, srcPath3), false), // Optimized by LLM: Extract creation of ResourceId lists
        createResourceIdList(ImmutableList.of(destPath1, destPath2, destPath3), false), // Optimized by LLM: Extract creation of ResourceId lists
  ...
  public void testCopySkipIfItExists() throws Exception {
    ...
    FileSystems.copy(
        createResourceIdList(ImmutableList.of(srcPath1, srcPath2), false), // Optimized by LLM: Extract creation of ResourceId lists
        createResourceIdList(ImmutableList.of(destPath1, destPath2), false), // Optimized by LLM: Extract creation of ResourceId lists
  ...
  private static List<ResourceId> createResourceIdList(List<Path> paths, final boolean isDirectory) { // Optimized by LLM: Extract creation of ResourceId lists
