...
public class ApproximateUnique {
  ...
  public static final class Globally<T> extends PTransform<PCollection<T>, PCollection<Long>> {
    ...
    // Optimized by LLM: Removed duplicate validation logic for sampleSize
    ...
    // Optimized by LLM: Removed duplicate validation logic for maximumEstimationError
    private static void validateMaximumEstimationError(double maximumEstimationError) {
    ...
    public Globally(double maximumEstimationError) {
      validateMaximumEstimationError(maximumEstimationError);
  ...
  public static final class PerKey<K, V>
    ...
    // Optimized by LLM: Removed duplicate validation logic for maximumEstimationError
    private static void validateMaximumEstimationError(double estimationError) {
    ...
    public PerKey(double estimationError) {
      validateMaximumEstimationError(estimationError);
  ...
  public static class ApproximateUniqueCombineFn<T>
    ...
    // Optimized by LLM: Catch specific exceptions instead of using Throwable
    @Override
    ...
    // Optimized by LLM: Use a more functional approach for merging accumulators
    @Override
    public LargestUnique mergeAccumulators(Iterable<LargestUnique> heaps) {
      LargestUnique accumulator = new LargestUnique(sampleSize);
      heaps.forEach(heap -> heap.heap.forEach(accumulator::add));
      return accumulator;
    }
    ...
    // Optimized by LLM: Handle IOException more gracefully
